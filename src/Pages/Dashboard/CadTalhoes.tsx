...
export default function CadTalhoes() {
  ...
  function montarDados() {
    ...
    const area = Number(formData.area_hectares);
    return {
      area,
      tipoCultura: culturas[String(formData.cultura_id)] || "Soja",
      idade: 1,
      volumeEstimado: area > 0 ? area * 2 : 1,
      idFazenda: 1,
    };
  }
  ...
  async function atualizarTalhao() {
    try {
      const response = await api.put(`/talhoes/${formData.id}`, montarDados());
      console.log("Talhão atualizado!", response.data, formData);
      alert("Talhão atualizado!");
    } catch (error) {
      console.error(error);
