...
export default function Culturas() {
  ...
  async function buscarCulturas() {
    try {
      ...
      let listaFormatada = response.data;
      if (response.data.length > 0 && Array.isArray(response.data[0])) {
        listaFormatada = response.data.map((item: any) => ({
          id: item[0],
          nome: item[1],
          safra: item[2]
        }));
  ...
  async function salvarCultura(e: React.FormEvent) {
    e.preventDefault();
    if (!formData.nome || !formData.safra) {
    ...
    try {
      ...
      setFormData({ nome: "", safra: "" }); 
      buscarCulturas(); 
    } catch (error) {
      console.error("Erro ao cadastrar cultura:", error);
      alert("Erro ao cadastrar cultura. Verifique o console.");
    }
  }
      ...
      <div className="flex flex-col lg:flex-row gap-8 mt-4">
        ...
        <div className="w-full lg:w-1/3 bg-white p-6 rounded-xl border border-gray-200 shadow-sm h-fit">
          ...
          <form onSubmit={salvarCultura} className="space-y-4">
            ...
            {/* Novo input para a Safra */}
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700">Safra</label>
              <input
                name="safra"
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white border-b border-gray-100 text-sm text-gray-500">
                  ...
                  <th className="py-3 px-6 font-medium">Safra</th>
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={4} className="text-center py-8 text-gray-500">Carregando dados...</td>
                  </tr>
                ) : culturas.length === 0 ? (
                  <tr>
                    <td colSpan={4} className="text-center py-8 text-gray-500">Nenhuma cultura cadastrada.</td>
                  </tr>
                ) : (
