...
// 1. Atualizamos a interface para aceitar as funções que vêm do componente pai
...
// 2. Extraímos onDelete e onEdit das props
export default function ListaTalhoes({ talhoes = [], onDelete, onEdit }: ListaTalhoesProps) {
  if (talhoes.length === 0) {
    return (
        ...
        {talhoes.map((talhao, index) => (
          ...
          >
            ...
            <div className="flex items-center gap-3 text-gray-400">
              ...
              {/* 3. Botão de Editar chama a função onEdit passando o ID do talhão */}
              <button 
                onClick={() => onEdit && onEdit(Number(talhao.id))}
              ...
              {/* 4. Botão de Excluir chama a função onDelete passando o ID do talhão */}
              <button 
                onClick={() => onDelete && onDelete(Number(talhao.id))}
